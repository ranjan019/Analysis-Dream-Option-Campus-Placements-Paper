...
def make_expected_stud_pref_list(company_final_ranklist):
	...
	company_order_8=company_order[280:300]
	...
	for i in range(0,1000):
		...
		student_pref_list.append(company_order_1+company_order_2+company_order_3+company_order_4+company_order_5+company_order_6+company_order_7+company_order_8)
...
def make_seq_stud_pref_list(company_final_ranklist, student_final_ranklist):
	...
	for i in range(0,len(student_final_ranklist)):
		...
		seq_part=[]
		non_seq_part=[]
		seq_part=company_order[int(i/float(10/3)):len(company_order)]
		non_seq_part=company_order[0:int(i/float(10/3))]
...
def cal_rank_efficiency(student_selected_list,stud_pref_list):
	...
	return float(float((sum_ranks)+300*(1000-total_students_placed))/1000)
